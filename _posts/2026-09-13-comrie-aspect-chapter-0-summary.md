---
layout: post
title: "Comrie, B. Aspect: An Introduction to the Study of Verbal Aspect and Related Problems(1995), 이철수, 박덕유 역(1998), 《동사 상의 이해》 0장 요약"
date: 2026-09-13 21:55:00 +0900
categories:
  - Linguistics
series: "Comrie 《Aspect》"
tags:
  - 상
  - aspect
  - Comrie
  - 시제
  - 완료상
  - 미완료상
  - perfective
  - imperfective
  - perfect
  - 문법화
---

<p class="citation-notice">
  이 글을 인용하거나 재사용할 때에는 출처를 밝혀 주시기 바랍니다.
  인용 정보:
  〈{{ page.title }}〉,
  《{{ site.title }}》,
  {{ page.date | date: "%Y-%m-%d" }},
  <a href="{{ page.url | absolute_url }}">{{ page.url | absolute_url }}</a>
</p>

이 글은 Bernard Comrie의 *Aspect: An Introduction to the Study of Verbal Aspect and Related Problems*의 제0장과 이철수·박덕유 역(1998) 《동사 상의 이해》의 해당 부분을 바탕으로, 필기 내용을 정리한 것이다. 0장의 목적은 상(aspect)의 세부 유형을 곧바로 분류하는 데 있지 않다. 먼저 상과 시제를 분리하고, 완료상(perfective)과 미완료상(imperfective)의 의미를 설정하며, 그 의미적 대립이 개별 언어에서 어떤 형식으로 문법화되는지를 구별한다. 또한 perfect와 perfective처럼 전통적으로 혼동되기 쉬운 용어를 정리하고, 이후 장에서 사용할 분석 원칙을 제시한다.

## 1. 상의 정의: 시제와 상, 완료상과 미완료상

상에 대한 출발점은 완료상과 미완료상의 대립이다. 러시아어의 *on pročital*과 *on čital*은 모두 영어로 *he read*라고 옮길 수 있지만, 미완료상은 문맥에 따라 *he was reading*, *he used to read*처럼 번역될 수 있다. 로망스어에서도 불어 *il lut : il lisait*, 스페인어 *leyó : leía*, 이탈리아어 *lesse : leggeva*와 같은 대립이 나타난다. 전통문법에서는 이들을 흔히 서로 다른 과거시제로 다루었지만, 콤리는 그 핵심을 상적 대립으로 본다.

{% gloss 1 "Russian: Perfective vs. Imperfective" %}
on pro-čital | ↔ | on čital
3SG.M.NOM PFV-read.PST.M.SG | ↔ | 3SG.M.NOM read.IPFV.PST.M.SG
‘그는 (다) 읽었다’ | ↔ | ‘그는 읽고 있었다 / 읽곤 했다’
{% endgloss %}

| 언어 | 완료상적 과거 | 미완료상적 과거 |
|---|---|---|
| 러시아어 | *on pročital* | *on čital* |
| 불어 | *il lut* | *il lisait* |
| 스페인어 | *leyó* | *leía* |
| 이탈리아어 | *lesse* | *leggeva* |

핵심은 **같은 과거시간 안에서도 사건을 하나의 전체로 보느냐, 내부의 진행·습관적 국면을 보느냐가 달라진다**는 점이다.

시제(tense)는 장면을 다른 시점, 보통 발화시와 관계시켜 시간축 위에 위치시키는 범주이다. *John is singing*은 발화시와 동시적이고, *John sang / John was singing*은 발화시보다 앞서며, *John will sing / John will be singing*은 발화시보다 뒤에 놓인다. 이런 의미에서 시제는 지시적(deictic)이다. 또한 콤리는 발화시와 직접 관계하는 절대시제와, 다른 사건의 시간에 기대어 해석되는 상대적 시간관계를 구별한다.

{% gloss 2 "English: Relative time reference" %}
When | walking | down | the road | I | often | meet Harry
when | walk.PTCP.PRS | down | DEF road | 1SG.NOM | often | meet.PRS Harry
‘길을 걷고 있을 때 나는 자주 해리를 만난다.’
{% endgloss %}

{% gloss 3 "English: Relative time reference in the past" %}
When | walking | down | the road | I | often | met Harry
when | walk.PTCP.PRS | down | DEF road | 1SG.NOM | often | meet.PST Harry
‘길을 걷고 있을 때 나는 자주 해리를 만났다.’
{% endgloss %}

두 예문에서 *walking*의 시간값은 스스로 현재나 과거를 정하지 않고 주절 *meet / met*와의 상대적 동시성으로 결정된다.

상(aspect)은 이와 달리 장면의 **“내적인 시간구성을 보는 여러 가지 방법”**이다. 따라서 콤리는 시제를 *situation-external time*, 상을 *situation-internal time*으로 대비한다.

| 구별 | 시제(tense) | 상(aspect) |
|---|---|---|
| 핵심 | 장면의 외적 시간 위치 | 장면의 내적 시간구성 |
| 기준 | 다른 시점, 보통 발화시 | 장면 자체의 시간적 내부 |
| 성격 | 지시적(deictic) | 사건을 바라보는 방식(viewpoint) |

이 차이는 다음과 같은 병렬 예문에서 가장 선명하다.

{% gloss 4 "English: Background and event" %}
John | was reading | when | I | entered
John | read.PROG.PST | when | 1SG.NOM | enter.PST
‘내가 들어갔을 때 John은 읽고 있었다.’
{% endgloss %}

{% gloss 5 "Russian: Background and event" %}
Ivan | čital | kogda | ja | vošel
Ivan.NOM | read.IPFV.PST.M.SG | when | 1SG.NOM | enter.PFV.PST.M.SG
‘내가 들어왔을 때 Ivan은 읽고 있었다.’
{% endgloss %}

{% gloss 6 "French: Background and event" %}
Jean | lisait | quand | j’entrai
Jean | read.IPFV.PST.3SG | when | 1SG.NOM=enter.PST.PFV.1SG
‘내가 들어왔을 때 Jean은 읽고 있었다.’
{% endgloss %}

{% gloss 7 "Spanish: Background and event" %}
Juan | leía | cuando | entré
Juan | read.IPFV.PST.3SG | when | enter.PRET.1SG
‘내가 들어왔을 때 Juan은 읽고 있었다.’
{% endgloss %}

{% gloss 8 "Italian: Background and event" %}
Gianni | leggeva | quando | entrai
Gianni | read.IPFV.PST.3SG | when | enter.PST.PFV.1SG
‘내가 들어왔을 때 Gianni는 읽고 있었다.’
{% endgloss %}

이 예들의 핵심은 **첫째 동사의 미완료상이 지속되는 장면을 배경(background)으로 제시하고, 둘째 동사의 완료상 사건이 그 배경 내부에 하나의 전체적 사건(event)으로 놓인다는 것**이다. *John was reading when I entered*에서 *was reading*은 독서 장면을 배경으로 펼쳐 놓고, *I entered*는 그 배경 내부의 한 지점에 놓인 하나의 전체적 사건으로 제시된다. 미완료상은 배경 장면의 시작과 끝을 직접 말하지 않지만, 완료상 사건은 그 내부 시간축에 위치한다.

완료상(perfective)은 장면을 바깥에서 바라보아 시작·중간·끝이라는 내부구조를 따로 구별하지 않고 하나의 전체로 제시한다. 반대로 미완료상(imperfective)은 장면의 내부로 들어가 그 진행 중인 국면이나 지속을 본다.

중요한 점은 완료성/미완료성이 사건 사이의 객관적 차이가 아니라는 것이다. 동일한 사건을 동일한 화자가 먼저 완료상적으로, 다시 미완료상적으로 제시해도 모순이 없다.

{% gloss 9 "English: The same event under two viewpoints" %}
John read that book yesterday | while | he was reading it | the postman came
John read.PST that book yesterday | while | 3SG.M.NOM read.PROG.PST 3SG.ACC | DEF postman come.PST
‘John은 어제 그 책을 읽었다.’ | ‘그가 그 책을 읽고 있는 동안’ | ‘우편배달부가 왔다.’
{% endgloss %}

첫 부분은 독서사건을 하나의 전체로, 뒤의 *was reading*은 동일한 사건의 내부 국면으로 제시한다.

완료상이 흔히 사건의 계기적 진행을 낳는 것도 그 자체의 정의와 구별해야 한다. *The wind tore off the roof, snapped the clothes-line, and brought down the apple-tree* 같은 문장은 세 사건이 제시된 순서대로 일어난 것으로 자연스럽게 해석되지만, 동시에 일어났을 가능성도 있다. 그러므로 “사건의 순차성”은 완료상의 직접적인 의미라기보다 전체적 사건 제시에서 파생되는 담화적 효과로 보아야 한다.

{% gloss 10 "English: Sequential reading as a discourse effect" %}
The wind | tore off the roof | snapped the clothes-line | and | brought down the apple-tree
DEF wind | tear.PST off DEF roof | snap.PST DEF clothes-line | and | bring.PST down DEF apple-tree
‘바람은’ | ‘지붕을 날려버렸고’ | ‘빨랫줄을 끊었으며’ | ‘그리고’ | ‘사과나무를 넘어뜨렸다.’
{% endgloss %}

## 2. 의미와 형식: 상적 의미와 문법화

콤리의 핵심 방법론은 **의미적 범주와 개별 언어의 형식**을 구별하는 데 있다. 모든 언어가 *today, the year before last, at five o’clock* 같은 어휘를 이용하여 시간을 표현할 수 있다고 해서 모두 문법적 시제를 갖는 것은 아니다. 시간관계가 문법적으로 체계화되어 있어야 그 언어가 시제를 가진다고 말할 수 있다. 상도 마찬가지이다. 완료상적·미완료상적 의미를 표현할 수 있다는 사실과, 그 대립이 특정한 문법범주로 문법화되어 있다는 사실은 구별해야 한다.

독일어의 예는 이를 잘 보여 준다. 일부 독일어 형식에서는 단순과거 *ich ging*가 완료형 *ich bin gegangen*에 의해 완전히 대체될 수 있으므로, 두 형태가 존재한다고 해서 *perfective : perfect*라는 상적 대립이 문법화되어 있다고 볼 수 없다. **Präteritumschwund**, 즉 구어에서 Präteritum이 점차 Perfekt에 의해 대체되어 Perfekt가 원래의 완료 의미뿐 아니라 일반적인 과거 서술까지 담당하게 되는 현상이 있다. 이 경우 형태상으로는 Präteritum과 Perfekt라는 두 형식이 알려져 있더라도 의미상 *perfective : perfect*의 대립이 보존된다고 할 수 없다. 현대 독일어에서 Präteritum의 사용은 특히 서면 글쓰기 쪽에 제한되는 경향이 있다.

{% gloss 11 "German: Präteritum vs. Perfekt" %}
ich ging | ↔ | ich bin gegangen
1SG.NOM go.PRET.1SG | ↔ | 1SG.NOM AUX.PRS.1SG go.PTCP.PST
‘I went’ | ↔ | ‘I have gone / I went’
{% endgloss %}

그럼에도 독일어는 문맥이나 다른 표현수단을 통해 영어 *he read the book*과 *he was reading the book*에 가까운 의미차를 나타낼 수 있다. 즉 **“상적 의미의 표현 가능성”과 “상 범주의 문법화”는 동일하지 않다.**

{% gloss 12 "German: Aspectual meaning without a general aspect inflection" %}
er | las | das Buch
3SG.M.NOM | read.PRET.3SG | DEF.ACC.N.SG book.ACC.SG
‘그는 그 책을 읽었다 / 읽고 있었다.’
{% endgloss %}

{% gloss 13 "German: A more progressive-like reading" %}
er | las | in=dem Buch
3SG.M.NOM | read.PRET.3SG | in=DEF.DAT.N.SG book.DAT.SG
‘그는 책 안에서(책의 일부를) 읽고 있었다.’
{% endgloss %}

핀란드어의 목적어 격 대립도 같은 문제를 보여 준다. *hän luki kirjan*과 *hän luki kirjaa*에서는 전체 목적어와 부분격의 차이가 상적 해석과 연결된다. 그러나 부분격의 기본적 의미는 대상의 일부가 영향을 받는다는 데 있으므로 **partitive = imperfective**로 일반화해서는 안 된다.

{% gloss 14 "Finnish: Total object" %}
hän | luki | kirjan
3SG.NOM | read.PST.3SG | book-GEN.SG[=TOTAL.OBJ]
‘그는 그 책을 (다) 읽었다.’
{% endgloss %}

{% gloss 15 "Finnish: Partitive object" %}
hän | luki | kirjaa
3SG.NOM | read.PST.3SG | book-PART.SG
‘그는 그 책을 읽고 있었다 / 읽었다(완독 여부 불명).’
{% endgloss %}

{% gloss 16 "Finnish: Partitive vs. total object" %}
hän otti ruokaa | ↔ | hän otti ruoan
3SG take.PST.3SG food-PART.SG | ↔ | 3SG take.PST.3SG food-GEN.SG[=TOTAL.OBJ]
‘그는 음식을 조금 취했다.’ | ↔ | ‘그는 그 음식을 전부 취했다.’
{% endgloss %}

{% gloss 17 "Finnish: Partitive in a completed event" %}
mies | ampui | lintua | pyrstöön
man.NOM.SG | shoot.PST.3SG | bird-PART.SG | tail-ILL.SG
‘그 남자는 새의 꼬리를 쐈다.’
{% endgloss %}

부분격은 미완료 의미와 자주 결합하지만, 완료된 사건에서도 대상의 일부만 영향을 받는 경우 나타날 수 있다. 따라서 특정 형식이 특정 상적 해석을 자주 유도한다고 해서 그 형식 자체를 곧바로 상 표지로 정의해서는 안 된다.

형식적 표현의 종류도 한정할 필요가 없다. 스페인어 *leyó : leía*처럼 굴절형태론적 대립일 수도 있고, 영어 *he read : he was reading*처럼 우언적(periphrastic) 구성일 수도 있다. 중요한 것은 상이 반드시 하나의 접사나 굴절어미로 표시되는가가 아니라 **의미적 대립이 체계적으로 형식화되어 있는가**이다.

또 실제 언어의 문법범주는 순수한 의미범주와 일대일 대응하지 않을 수 있다. 예를 들어 스페인어 *Juan leía*는 미완료상적 의미만이 아니라 과거 시간관계도 함께 표현한다. 즉 하나의 형식 속에 tense와 aspect가 결합될 수 있다. 그러므로 보편적 의미범주를 분석할 때는 특정 언어의 전통적인 범주명과 그 형식이 실제로 지니는 의미를 분리해야 한다.

### Gesamtbedeutung과 Grundbedeutung

이상적인 분석은 한 형식의 여러 개별 용법을 하나의 일반적 의미로부터 예측하는 것이다. 이처럼 모든 용법을 관통하는 일반 의미를 **Gesamtbedeutung**이라 한다. 그러나 하나의 일반 의미가 반드시 존재한다고 미리 가정해서는 안 된다. 한편 여러 의미 중 중심적이고 전형적인 의미를 **Grundbedeutung**(기본 의미)이라 하며, 주변적·확장된 의미는 *Nebenbedeutungen*으로 다룰 수 있다.

| 용어 | 뜻 |
|---|---|
| Gesamtbedeutung | 한 형식의 모든 용법을 관통하는 일반 의미 |
| Grundbedeutung | 여러 의미 가운데 중심적·기본적인 의미 |
| Nebenbedeutungen | 기본 의미에서 주변적으로 확장된 의미들 |

의미 확대와 문법화의 통시적 예로 로망스어의 perfect 발달이 제시된다. 원래 perfect는 과거 사건을 현재와의 관계 속에서 기술했지만, 그 사용조건이 점차 약화되면서 일반 과거로 영역을 넓혀 단순과거를 대체할 수 있다. 웨일즈어에서는 본래 진행상적 의미의 우언형이 점차 현재의 미완료상 영역을 넓히는 과정이 관찰된다. 따라서 한 형식의 “기본 의미”와 실제 사용범위는 역사적으로 달라질 수 있다.

## 3. 용어법: perfective와 perfect를 구별할 것

0장에서 가장 강하게 경고되는 용어 혼동은 **perfective와 perfect의 동일시**이다. perfective는 imperfective와 대립하며 장면을 내부 시간구성의 구별 없이 하나의 전체로 보는 방식이다. perfect는 이와 다른 범주로, 선행하는 과거 장면을 현재와 관련짓는 의미를 가진다.

{% gloss 18 "English: Perfect vs. simple past" %}
John has read the book | ↔ | John read the book
John AUX.PRS.3SG read.PTCP.PST DEF book | ↔ | John read.PST DEF book
‘John은 그 책을 읽은 적이 있다 / 읽어 놓았다.’ | ↔ | ‘John은 그 책을 읽었다.’
{% endgloss %}

{% gloss 19 "Spanish: Perfect vs. preterite" %}
Juan ha leído el libro | ↔ | Juan leyó el libro
Juan AUX.PRS.3SG read.PTCP.PST DEF.M.SG book | ↔ | Juan read.PRET.3SG DEF.M.SG book
‘Juan은 그 책을 읽었다(현재와 관련된 완료).’ | ↔ | ‘Juan은 그 책을 읽었다(완결된 과거 사건).’
{% endgloss %}

{% gloss 20 "English: Present result of a prior event" %}
His arm | has been broken
3SG.M.POSS arm | AUX.PRS.3SG be.PTCP.PST break.PTCP.PST
‘그의 팔이 부러져 있다 / 부러진 상태다.’
{% endgloss %}

즉 **perfect는 과거 사건과 현재의 관련성을 문제 삼고, perfective는 사건 내부를 전체로 조망하는 방식을 문제 삼는다.**

불어의 *Jean a lu le livre*와 *Jean lut le livre*처럼 전통적으로 perfect와 정과거가 대립하는 경우가 있지만, 구어에서는 복합과거가 단순과거의 영역까지 확대되어 원래의 대립이 약화될 수 있다. 따라서 형태의 전통적 명칭만으로 의미를 단정해서는 안 된다.

*Aorist*라는 용어도 주의해야 한다. 일부 전통에서 aorist는 과거시제의 perfectivity를 가리키므로 보편적인 perfective와 완전히 같지 않다. 고대그리스어에서는 직설법 Aorist가 일차적으로 과거시제적이지만, 다른 서법과 비정형 형식에서는 시제보다 상적 대립이 두드러진다. 마찬가지로 라틴어 Perfect는 *necāvī*가 “I killed”와 “I have killed”를 모두 나타낼 수 있어 의미론적 perfect와 일치하지 않는다.

{% gloss 21 "Latin: Traditional Perfect" %}
necāvī
necāv-ī
kill-PRF.1SG
‘I killed’ / ‘I have killed’
{% endgloss %}

진행상(progressive)과 지속상(continuous)도 구별된다. 콤리의 용어법에서 진행성은 지속성과 비상태성(nonstativity)의 결합이며, 따라서 progressive는 continuous의 하위범주이다. 영어 *John was reading*은 전형적인 진행상 예이다.

끝으로 콤리는 상태(state), 사건(event), 과정(process)을 포괄하기 위해 **situation**이라는 일반 용어를 사용한다. 0장에서는 상태를 정적인 장면, 사건과 과정을 동적인 장면으로 두고, 과정은 미완료상적으로 내부 진행이 보이는 동적 장면, 사건은 완료상적으로 하나의 전체로 보이는 동적 장면으로 잠정 구별한다. 세부적인 어휘적 상의 문제는 제2장에서 본격적으로 다룬다.

## 정리

| 대립 | 핵심 |
|---|---|
| tense : aspect | 장면의 외적 시간 위치 : 장면의 내적 시간구성 |
| perfective : imperfective | 전체적 조망 : 내부적 조망 |
| perfective : perfect | 사건을 전체로 보는 방식 : 선행사건과 기준시의 관련 |
| 상적 의미 : 문법적 Aspect | 의미론적 대립 : 그 대립의 체계적 문법화 |
| Gesamtbedeutung : Grundbedeutung | 모든 용법을 포괄하는 일반 의미 : 중심적·기본적 의미 |
| 보편 의미범주 : 언어별 범주명 | 분석적 의미 개념 : 개별 언어의 전통적·형식적 명칭 |

> 콤리에게 상은 “사건이 실제로 끝났는가”를 단순히 표시하는 범주가 아니라, 화자가 situation의 내적 시간구조를 어떤 방식으로 제시하는가에 관한 의미론적 범주이다. 그리고 이 의미적 대립이 개별 언어에서 어떤 형식으로, 어느 정도까지 문법화되어 있는지는 별도로 검토해야 한다.

### 글로싱 약호

1/3 = 1·3인칭, SG = 단수, M/N = 남성/중성, NOM/ACC/DAT/GEN = 주격/대격/여격/속격, PART = 부분격, ILL = 향내격, PST = 과거, PRET = 단순과거(pretérito), PRS = 현재, PRF = 완료형, PFV/IPFV = 완료상/미완료상, PROG = 진행, PTCP = 분사, AUX = 조동사, DEF/DEM/POSS = 정관사/지시사/소유.

### 참고문헌

Comrie, Bernard. *Aspect: An Introduction to the Study of Verbal Aspect and Related Problems*. Cambridge: Cambridge University Press, 1995.

Comrie, Bernard. 이철수·박덕유 역. 《동사 상의 이해》. 1998.
