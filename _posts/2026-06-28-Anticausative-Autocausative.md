---
layout: post
title: "반사동(Anticausative)과 자기사동(Autocausative)"
date: 2026-06-28 21:20:00 +0900
categories:
  - Linguistics

tags:
  - 형태론
  - 통사론
  - 사동성
  - 반사동
  - 자기사동
  - anticausative
  - autocausative
  - valency
---

<p class="citation-notice">
  이 글을 인용하거나 재사용할 때에는 출처를 밝혀 주시기 바랍니다.
  인용 정보:
  「{{ page.title }}」,
  『{{ site.title }}』,
  {{ page.date | date: "%Y-%m-%d" }},
  <a href="{{ page.url | absolute_url }}">{{ page.url | absolute_url }}</a>
</p>

반사동사(anticausative verb)는 주어에 영향을 미치는 사건을 나타내는 나타내는 자동사이면서 그 사건의 원인에 대해 의미적으로도 통사적으로도 아무런 표시를 하지 않는 동사.
반사동사의 유일한 논항은 그것의 주어로, 수동자(patient), 즉 어떤 행위를 겪는 대상이다. 원인(cause)이나 사동주(causer)가 있다고 가정할 수는 있지만 반사동사의 통사 구조상 그것을 직접 언급하는 것은 부자연스럽거나 불가능하다.
반사동사의 예는 <i>break</i>, <i>sink</i>, <i>move</i> 등이 있다.
반사동사는 비대격동사(unaccusative verbs)의 하위 부류이다. 두 용어가 대체로 동의어처럼 쓰이기는 하지만, 어떤 비대격동사는 더 명백히 반사동적이고, <i>fall</i>, <i>die</i>와 같은 다른 동사들은 그렇지 않다.
이는 사동성(causation)을 "의지를 가진 유정행위자"와 관련된 것으로 정의하느냐의 여부에 달려 있다. 

반사동사와 자기사동사(autocausative verb)는 구분되어야 한다. 행위자가 명시되지는 않았지만 외부에 있다고 가정되거나, 그 존재가 심지어 부정될 수 있는 경우 그 동사는 반사동사이다. 
반면 행위자(agent)가 수동자(patient)와 동일한 경우에는 자기사동사이다. 많은 인도유럽어는 이 두 부류를 구별하는 별도의 형태론적 표지들을 가지고 있지 않으므로 어느 부류에 속하는지는 문맥을 통해 파악해야 한다. 

자기사동사(autocausative verb)는 재귀(reflexive)의 한 유형이며, "많은 경우 *(인간) 지시체(referent)가 자신의 활동을 통해 야기하는 위치의 변화나 운동"을 가리킨다[^1]. 
이러한 사건에서는 "유정물의 정신이나 의지가 자기 몸 또는 그 일부의 운동을 시작한다."[^2] 보통 유정적인 "주어로 표시된 지시체는 행위자의 활동성을 지니는 동시에 수동자/주어처럼 상태의 변화를 겪는다." [^3]

다음은 영문판 위키피디아(Anticausative verb) 문서에서 예시를 가져온 것이다. 

{% gloss 1 "Lithuanian: Anticausative" %}
Vežim-ėl-is | at-si-riš-o | nuo | krūm-o. 
cart-DIM-NOM.SG | PVB-REFL-tie-PST.3 | from | bush-GEN.SG 
'The cart got untied from the bush.'
{% endgloss %}

{% gloss 2 "Lithuanian: Autocausative" %}
Arklys | at-si-riš-o | nuo | krūm-o. 
horse.NOM.SG | PVB-REFL-tie-PST.3 | from | bush-GEN.SG 
'The horse got itself untied from the bush.'
{% endgloss %}

{% gloss 3 "Russian: Anticausative %}
Чашк-а | упа-л-а | со | стол-а | и | раз-би-л-а-сь. |
Čašk-a | upa-l-a | so | stol-a | i | raz-bi-l-a-sʹ 
cup-NOM.SG | fall-PST-F.SG | from | table-GEN | and | PVB-break-PST-F.SG-REFL 
'The cup fell from the table and crashed (itself).'
{% endgloss %}


{% gloss 4 "Russian: Autocausative" %}
Водитель | раз-би-л-ся | на | горн-ой | дорог-е.  
Voditelʹ | raz-bi-l-sja | na | gorn-oj | dorog-e. 
driver.NOM | PVB-break-PST.M.SG-REFL | on | mountain-LOC.SG | road-LOC.SG 
'The driver crashed (himself) on a mountain road.'
{% endgloss %}

# 주석

[^1]: Geniušienė, Emma (1987). The Typology of reflexives. Empirical approaches to language typology. Berlin: Mouton de Gruyter. ISBN 978-3-11-010677-0.

[^2]: Gaby, Alice (2023-06-13), "Reflexives and reciprocals", in Bowern, Claire (ed.), The Oxford Guide to Australian Languages (1 ed.), Oxford University PressOxford, pp. 360–377, doi:10.1093/oso/9780198824978.003.0031, ISBN 978-0-19-882497-8, retrieved 2023-12-05

[^3]: Parry, Mair (1998). "The reinterpretation of the reflexive in Piedmontese: 'impersonal' SE constructions". Transactions of the Philological Society. 96 (1): 63–116. doi:10.1111/1467-968X.00024. ISSN 0079-1636.
